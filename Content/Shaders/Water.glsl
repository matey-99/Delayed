...
layout (location = 25) uniform sampler2DArray u_DirectionalLightShadowMaps;
layout (location = 26) uniform vec3 u_SkyLightColor;
...
void main()
{
    ...
    vec2 flow1 = (texture(u_Material.flowMap, vec2(v_TexCoord.x + u_FrameTime * timeScale, v_TexCoord.y)).rg * 2.0 - 1.0) * u_Material.waveStrength;
    vec2 flow2 = (texture(u_Material.flowMap, vec2(-v_TexCoord.x + u_FrameTime * timeScale, v_TexCoord.y + u_FrameTime * timeScale)).rg * 2.0 - 1.0) * u_Material.waveStrength;
    vec2 flow = flow1 + flow2;

    if (u_Material.isAlbedoMap)
    albedo = pow(texture(u_Material.albedoMap, v_TexCoord + flow).rgb, vec3(2.2));
    else
