...
void Footsteps::Start() {
    ...
    m_CharacterController = m_Owner->GetScene()->GetComponent<CharacterController>(m_PlayerID);
    m_Player = m_Owner->GetScene()->GetComponent<Player>(m_PlayerID);
}

void Footsteps::Update(float deltaTime) {

    if (m_CharacterController->IsJumping()) {
        m_AudioSource->ChangeSound(m_Sounds.at(10));
        m_AudioSource->PlaySound();
    }

    if (m_CharacterController->GetMovementSpeed() > 0.1f && m_CharacterController->IsGrounded()) {
